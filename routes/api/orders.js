...
router.post('/filter', (req, res) => {
  ...
  if(req.body.authentication > 9) {
    ...
    Order.find({department: req.body.department})
      .sort({ insertDate: -1 })
      .then(items => res.json(items));
  } else if(req.body.authentication === 8) {
    Order.find({department: req.body.department})
      .sort({insertDate: -1})
      .then(items => res.json(items));
  } else {
...
router.post('/getLog', (req, res) => {
  Log.find({"inputItem[0].id" : req.body.id})
    .sort({ insertDate: -1 })
    .then(items => res.json(items));
})
...
router.post('/update', (req, res) => {
  ...
  const logItem = new Log({user_id:"123", lastname: "1233", modify:"UploadImage", coll:"order", verify: "Upload Image", "inputItem":req.body});logItem.save();
  const updateValue = {$set: od}
  Order.updateOne({ "order_id":req.body.id}, updateValue)
    .then(items => res.json(items));
})
